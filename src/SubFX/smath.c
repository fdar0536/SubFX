...
subfx_exitstate subfx_math_init(subfx_math_api *math)
{
    if (!math)
    {
        return subfx_failed;
    ...
    math->arc_curve = subfx_math_arc_curve;
    math->bezier = subfx_math_bezier;
    math->degree = subfx_math_degree;
    math->distance = subfx_math_distance;
    math->line_intersect = subfx_math_line_intersect;
    math->ortho = subfx_math_ortho;
    math->randomsteps = subfx_math_randomsteps;
    math->round = subfx_math_round;
    math->stretch = subfx_math_stretch;
    math->trim = subfx_math_trim;
    math->ellipse = subfx_math_ellipse;
    math->randomway = subfx_math_randomway;
    math->rotate = subfx_math_rotate;
                      ...
                      char *errMsg)
{
    ...
    if (!fdsa)
    {
        subfx_pError(errMsg, "arc_curve: You should never see this line.")
        return NULL;
    }
    ...
    fdsa_vector *curves = fdsa->vector.create(sizeof(double));
    if (!curves)
    {
    ...
    while (angle_sum < angle)
    {
        ...
        ptr = subfx_math_rotate2d(rx0, ry0,
                                  cw * 90. * cur_angle_pct);
        checkError;
        ...
        tmpDouble = subfx_math_distance(rx03, ry03, 0.);
        tmpDouble = pow(tmpDouble, 2) / 2.;
        tmpDouble = sqrt(tmpDouble);

        ptr = subfx_math_stretch(rx03, ry03, 0,
                                 tmpDouble * kappa);
        checkError;
        ...
        ptr = subfx_math_rotate2d(rx03, ry03,
                                  cw * (-45.) * cur_angle_pct);
        checkError
        ...
        ptr = subfx_math_rotate2d(rx03 * -1., ry03 * -1.,
                                  cw * 45. * cur_angle_pct);
        checkError;
...
{
    double degree = subfx_math_distance(x1, y1, z1) *
                    subfx_math_distance(x2, y2, z2);
    if (degree == 0.)
    {
        return 0.;
    }
...
*subfx_math_stretch(double x, double y, double z, double length)
{
    ...
    double cur_length = subfx_math_distance(x, y, z);
    if (cur_length == 0.)
    {
        return ret;
    }
